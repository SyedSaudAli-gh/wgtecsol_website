...
export default function Navbar() {
  ...
  return (
    <>
      <header className={`fixed inset-x-0 top-0 z-50 h-16 sm:h-20 md:h-24 bg-black/70 backdrop-blur border-b border-white/10 ${
        ...
        <nav className="mx-auto max-w-7xl h-full px-4 flex items-center justify-between">
          ...
          {/* Logo */}
          <Link href="/" className="flex items-center z-10 -space-x-4 sm:-space-x-8 sm:gap-2">
            <Image
              src="/images/Logo.png"
              alt="Logo"
              width={121}
              height={77}
              className="h-[50px] w-[78px] sm:h-[60px] sm:w-[94px] md:h-[77px] md:w-[121px] object-contain"
            />
          ...
          <div className="hidden lg:block z-10">
            <Link
              href="/contact"
              className={`px-4 xl:px-5 py-2 xl:py-3 font-medium rounded-lg transition-all duration-200 text-sm xl:text-base ${
                pathname === '/contact'
                  ? 'bg-[#9EFF00] text-black ring-2 ring-white'
                  : 'bg-[#9EFF00] text-black hover:bg-[#8CE600] hover:scale-105'
              }`}
            >
              Contact Us
            </Link>
      ...
      <div className={`lg:hidden fixed inset-0 bg-black/95 backdrop-blur-md transition-transform duration-300 z-40 ${
        ...
        <div className="flex flex-col h-[calc(100%-62px)] sm:h-[calc(100%-76px)] md:h-[calc(100%-93px)]">
            ...
            <div className="flex flex-col items-stretch px-4 sm:px-6 py-6 space-y-2">
              {navItems.map((item) => (
                <Link
                  ...
                  className={`w-full text-center py-3 sm:py-4 font-medium rounded-lg transition-all duration-200 text-base sm:text-lg ${
                    pathname === item.href
          ...
          <div className="px-4 sm:px-6 pb-6">
            <Link
              href="/contact"
              onClick={() => setOpen(false)}
              className={`block w-full text-center py-3 sm:py-4 font-medium rounded-lg transition-all duration-200 text-base sm:text-lg ${
                pathname === '/contact'
                  ? 'bg-[#9EFF00] text-black ring-2 ring-white'
